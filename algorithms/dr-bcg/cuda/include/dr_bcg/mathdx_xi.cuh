...
// MathDx-fused reduced-system ( xi) chain for DR-BCG (PLAN.md Stage 3, K2-K4).
//
// Replaces the host-API path that, per iteration, forms the Gram G = s^T*A*s,
// inverts it explicitly with an LU factorization (cuSolver getrf/getrs), and
// applies the inverse with two cuBLAS GEMMs -- and which recomputes A*s a
// second time for the w update. Instead, with AS = A*s computed once (held by
// the solver as a cuSPARSE SpMM boundary, K1), this fuses:
//
//   K2  G = s^T * AS                       (cuBLASDx tiled Gram)
//   K3  G = F^T F, F = chol(G)             (cuSolverDx POTRF; info -> breakdown)
//   K4  apply G^-1 twice:
//         C = G^-1 * sigma -> X += s*C     (X update)
//         U = AS * G^-1                    (feeds the w update; = A*s*xi)
//
// "Factor once, apply twice" (PLAN.md design note 2): G is factored a single
// time; G^-1 is formed from the well-conditioned Cholesky factor as
// Ginv = Finv*Finv^T (Finv the triangular inverse of F), reusing the same
// trinv/apply pattern as the Stage-2 CholeskyQR2 backend rather than the old
// LU-of-G inverse. Eliminating the second A*s SpMM is the structural win
// (A*(s*xi) = (A*s)*xi = AS*G^-1).
//
// Reuses the kernels/descriptors/sizing helpers from mathdx_qr.cuh
// (dr_bcg::mathdx_detail): GemmOp, PotrfOp, enable_dynamic_smem, potrf_kernel,
// trinv_kernel, apply_kernel, rmul_kernel. Compiles only when
// SOLVERS_BUILD_MATHDX is defined (built in the cuda_dr_bcg_mathdx target).
...
#include "dr_bcg/mathdx_qr.cuh" // shared detail kernels + descriptors
...
// small N*N cuBLASDx GEMM. Negligible (N <= 64) done once per iteration.
// ---------------------------------------------------------------------------
...
void launch_xi(T *d_s, T *d_AS, T *d_sigma, T *d_X, T *d_U, int m, int ld,
    ...
    // K3: F = chol(G); Finv = F^-1; Ginv = Finv*Finv^T = G^-1
