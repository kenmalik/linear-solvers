...
// MathDx-fused CholeskyQR2 orthonormalization policy for DR-BCG (PLAN.md Stage 2).
//
// Replaces the host-API Gram->POTRF->TRSM sequence of CholeskyQr (qr.cuh) with
// device-side fused kernels built from cuBLASDx (block GEMM) and cuSolverDx
// (block POTRF). The pass is run twice (CholQR2) because a single CholQR squares
// the condition number of the input; the two triangular factors are combined so
// the returned R is the true factor with A = Q * R.
//
// Per pass the triangular factor R = chol(Gram) (cuSolverDx POTRF) is inverted
// in shared memory (tiny N*N, single block) and the Q-panel update Q = A * R^-1
// is applied as a tiled cuBLASDx GEMM. Forming the N*N inverse once is negligible
// (PLAN.md design note 2) and keeps the large m-tiled work in cuBLASDx GEMMs.
//
// This file compiles only when SOLVERS_BUILD_MATHDX is defined (the build links
// mathdx::cublasdx + mathdx::cusolverdx and defines the macro on dr_bcg::cuda).
// Written against MathDx 25.12 (cublasdx 0.5.x / cusolverdx); FillMode/SM are set
// explicitly per PLAN.md to survive API-default churn.
...
#include "dr_bcg/qr.cuh" // QrPolicy concept (documentation of the interface)
...
// Target architecture. A100 (SM 8.0); matches CMAKE_CUDA_ARCHITECTURES=80.
inline constexpr unsigned MATHDX_CHOLQR2_SM = 800;

// Row-tile height for the two panel GEMMs (Gram and apply). The k*k operands and
// a TILE*k panel must co-reside in SRAM; 128 rows at k<=64 fp64 fits A100 smem.
// Tune with the Nsight Compute occupancy calculator (PLAN.md sizing notes).
inline constexpr int MATHDX_TILE_ROWS = 128;

// Threads per block for the cuBLASDx GEMM kernels.
inline constexpr int MATHDX_GEMM_BLOCK_DIM = 128;
...
void enable_dynamic_smem(Kernel kernel, unsigned bytes) {
    if (bytes > 48 * 1024) {
...
using GemmOp =
    decltype(cublasdx::Size<M, N, K>() + cublasdx::Precision<T>() +
             ...
             cublasdx::BlockDim<MATHDX_GEMM_BLOCK_DIM>());
...
// ---------------------------------------------------------------------------
template <class T, int N>
__global__ __launch_bounds__(PotrfOp<T, N>::max_threads_per_block) void
potrf_kernel(const T *__restrict__ G, T *__restrict__ R,
...
void launch_cholqr2(T *d_Q, T *d_R, const T *d_A, int m, int ldA, T *d_G,
                    ...
                    cudaStream_t stream) {
    constexpr int TILE = MATHDX_TILE_ROWS;
...
// QR policy satisfying the QrPolicy<T> concept (qr.cuh) using fused MathDx
// CholeskyQR2. Drops into dr_bcg::cuda::solve as the Qr template argument with no
// changes to the solver loop.
...
class MathDxCholeskyQr2 {
  public:
    ...
    MathDxCholeskyQr2(cusolverDnHandle_t & /*cusolverH*/,
                      cusolverDnParams_t & /*params*/, int /*m*/, int n)
        : block_size(n) {
        CUDA_CHECK(cudaMalloc(&d_G, sizeof(T) * n * n));
        CUDA_CHECK(cudaMalloc(&d_R1, sizeof(T) * n * n));
        CUDA_CHECK(cudaMalloc(&d_R2, sizeof(T) * n * n));
        CUDA_CHECK(cudaMalloc(&d_Rinv, sizeof(T) * n * n));
    ...
    void solve(T *&d_Q, T *&d_R, const T *d_A, const int &m, const int &n,
        ...
        dispatch(d_Q, d_R, d_A, m, n, stream);
  ...
  private:
    void dispatch(T *d_Q, T *d_R, const T *d_A, int m, int n,
                  cudaStream_t stream) {
        using dr_bcg::mathdx_detail::launch_cholqr2;
        switch (n) {
        case 1:
            launch_cholqr2<T, 1>(d_Q, d_R, d_A, m, m, d_G, d_R1, d_R2, d_Rinv, d_info, stream);
            break;
        case 2:
            launch_cholqr2<T, 2>(d_Q, d_R, d_A, m, m, d_G, d_R1, d_R2, d_Rinv, d_info, stream);
            break;
        case 4:
            launch_cholqr2<T, 4>(d_Q, d_R, d_A, m, m, d_G, d_R1, d_R2, d_Rinv, d_info, stream);
            break;
        case 8:
            launch_cholqr2<T, 8>(d_Q, d_R, d_A, m, m, d_G, d_R1, d_R2, d_Rinv, d_info, stream);
            break;
        case 16:
            launch_cholqr2<T, 16>(d_Q, d_R, d_A, m, m, d_G, d_R1, d_R2, d_Rinv, d_info, stream);
            break;
        case 32:
            launch_cholqr2<T, 32>(d_Q, d_R, d_A, m, m, d_G, d_R1, d_R2, d_Rinv, d_info, stream);
            break;
        case 64:
            launch_cholqr2<T, 64>(d_Q, d_R, d_A, m, m, d_G, d_R1, d_R2, d_Rinv, d_info, stream);
            break;
        default:
            throw std::runtime_error(
